...
const userApprovalStreamClients = new Map();
const FCM_TOKEN_URL = "https://oauth2.googleapis.com/token";
const FCM_SCOPE = "https://www.googleapis.com/auth/firebase.messaging";
let cachedFcmAccessToken = null;
function fcmEnabled() {
    return Boolean(String(env.FCM_PROJECT_ID ?? "").trim() &&
...
async function disablePushToken(token) {
    await pgPool.query("UPDATE user_push_tokens SET is_active = FALSE, updated_at = NOW() WHERE token = $1", [token]);
}
function pushMessageForEvent(eventType, payload) {
    ...
    if (eventType === "insurance_status") {
        ...
        const claimType = claimTypeRaw === "vehicle_damage"
            ? "Vehicle Damage"
...
async function sendPushForUserEvent(userId, eventType, payload) {
    if (!fcmEnabled())
        return;
    const accessToken = await getFcmAccessToken();
    if (!accessToken)
        return;
    const r = await pgPool.query("SELECT token FROM user_push_tokens WHERE user_id = $1 AND is_active = TRUE ORDER BY updated_at DESC", [userId]);
    if (!r.rowCount)
        return;
...
const AVAILABLE_PLATFORMS = ["zomato", "blinkit", "rapido", "ola"];
const MAX_PLATFORMS = AVAILABLE_PLATFORMS.length;
async function getCatalogPlatformCapacity() {
    const r = await pgPool.query("SELECT COUNT(*)::int AS c FROM integration_platform_catalog");
    const count = Number(r.rows[0]?.c ?? 0);
    return count > 0 ? count : MAX_PLATFORMS;
}
function publishPlatformCatalogChanged() {
...
function publishUserApprovalUpdate(userId, eventType, payload) {
...
function deriveTaxChatTitle(input) {
...
function monthlyPurchaseCap(plan, maxPlatforms) {
    const p = plan.trim().toLowerCase();
    if (p === "solo")
        return maxPlatforms;
    if (p === "duo")
        return Math.floor(maxPlatforms / 2);
    if (p === "trio")
        return Math.floor(maxPlatforms / 3);
    // Legacy support for previously purchased plans.
    if (p === "unity")
        return Math.floor(maxPlatforms / 6);
    return 0;
}
async function _loadPlanWindows(userId) {
    const r = await pgPool.query(`SELECT id, plan, created_at, (created_at + INTERVAL '30 days') AS expires_at
...
async function refreshSubscriptionRuntimeState(userId) {
    const windows = await _loadPlanWindows(userId);
    const maxPlatforms = await getCatalogPlatformCapacity();
    await pgPool.query(`UPDATE platform_subscription_bindings b
       SET unbound_at = NOW()
    ...
    await pgPool.query(`DELETE FROM platform_connections pc
      WHERE pc.user_id = $1
        AND NOT EXISTS (
          SELECT 1
    ...
    const usedR = await pgPool.query(`SELECT COUNT(DISTINCT lower(platform))::int AS c
     FROM platform_subscription_bindings b
     JOIN subscription_purchases sp ON sp.id = b.purchase_id
     WHERE b.user_id = $1
       AND b.unbound_at IS NULL
       AND b.expires_at > NOW()
       AND (sp.created_at + INTERVAL '30 days') > NOW()`, [userId]);
    const used = Number(usedR.rows[0]?.c ?? 0);
    const historyR = await pgPool.query(`SELECT DISTINCT ON (lower(platform)) platform
     FROM platform_subscription_bindings b
     JOIN subscription_purchases sp ON sp.id = b.purchase_id
     WHERE b.user_id = $1
       AND b.unbound_at IS NULL
       AND b.expires_at > NOW()
       AND (sp.created_at + INTERVAL '30 days') > NOW()
     ORDER BY lower(platform), b.bound_at ASC`, [userId]);
    const historyPlatforms = historyR.rows.map((r) => String(r.platform));
    const limit = Math.min(maxPlatforms, windows.active.reduce((a, w) => a + w.slots, 0));
    ...
    const activePlanExpiresAt = windows.active.length
        ...
        : null;
    const status = limit > 0 ? "active" : "inactive";
    await pgPool.query(`INSERT INTO subscriptions (user_id, active_plan, status, plan_limit, used, active_plan_expires_at, updated_at)
...
function getAdminTokenPayload(req) {
    const auth = String(req.header("authorization") ?? "");
    if (!auth.startsWith("Bearer "))
        return null;
    const token = auth.slice(7).trim();
    ...
    try {
        const payload = jwt.verify(token, env.JWT_SECRET);
        if (!(payload?.admin === true || String(payload?.role ?? "").toLowerCase() === "admin"))
            return null;
...
function adminActorFrom(req, fallback = "admin") {
...
async function logAdminActivity(req, action, details = {}, actorOverride) {
    ...
    try {
        await pgPool.query("INSERT INTO admin_activity_logs (actor_username, action, details, created_at) VALUES ($1,$2,$3::jsonb,NOW())", [actor, String(action || "unknown"), JSON.stringify(details || {})]);
    }
    catch (error) {
...
function prettyPlatformName(slug) {
    return slug
        .split("-")
        .filter(Boolean)
...
function normalizeMicroInsuranceType(input) {
...
function annualLoanRatePercentByConditions(conditionsMet) {
    if (conditionsMet >= 3)
        return 7;
    if (conditionsMet === 2)
        return 10;
...
function calculateReducingLoanPlan(amount, annualRateDecimal) {
...
let insuranceClaimsColumnsEnsuring = null;
async function ensureInsuranceClaimsColumns() {
    if (insuranceClaimsColumnsEnsured)
        return;
    if (insuranceClaimsColumnsEnsuring)
    ...
    insuranceClaimsColumnsEnsuring = (async () => {
        ...
        await pgPool.query("ALTER TABLE insurance_claims ADD CONSTRAINT insurance_claims_claim_type_check CHECK (claim_type IN ('vehicle_damage','product_damage_loss')) NOT VALID");
...
app.post("/admin/login", async (req, res) => {
    ...
    const r = await pgPool.query("SELECT id, username, email, password_hash FROM admin_users WHERE lower(username) = lower($1) LIMIT 1", [username]);
    if (!r.rowCount) {
    ...
    if (!ok)
        return res.status(401).json({ message: "Invalid credentials", detail: "Invalid credentials" });
    const token = jwt.sign({ admin: true, role: "admin", adminId: admin.id, email: admin.email, username: admin.username }, env.JWT_SECRET, { expiresIn: "12h" });
    res.json({ token, admin: { username: admin.username, email: admin.email } });
});
app.post("/admin/password/request-otp", async (req, res) => {
    const username = String(req.body?.username ?? "").trim().toLowerCase();
    if (!username)
        return res.status(400).json({ message: "Username required", detail: "Username required" });
    const r = await pgPool.query("SELECT id, username FROM admin_users WHERE lower(username) = lower($1) LIMIT 1", [username]);
    if (!r.rowCount)
        return res.status(404).json({ message: "Admin not found", detail: "Admin not found" });
    const otp = genOtp();
...
app.post("/admin/password/verify-otp-change", async (req, res) => {
    ...
    const otpR = await pgPool.query("SELECT otp, expires_at FROM admin_password_reset_otps WHERE lower(username) = lower($1) LIMIT 1", [username]);
    if (!otpR.rowCount)
        return res.status(400).json({ message: "Invalid or expired OTP", detail: "Invalid or expired OTP" });
    const found = otpR.rows[0];
    const valid = found.otp === otp && new Date(found.expires_at).getTime() >= Date.now();
    if (!valid)
        return res.status(400).json({ message: "Invalid or expired OTP", detail: "Invalid or expired OTP" });
    const adminR = await pgPool.query("SELECT id FROM admin_users WHERE lower(username) = lower($1) LIMIT 1", [username]);
    if (!adminR.rowCount)
        return res.status(404).json({ message: "Admin not found", detail: "Admin not found" });
...
app.post("/admin/password/change", requireAdminKey, async (req, res) => {
    ...
    if (!ok)
        return res.status(401).json({ message: "Invalid old password", detail: "Invalid old password" });
    const nextHash = await bcrypt.hash(newPassword, 10);
...
app.get("/me", async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    const r = await pgPool.query("SELECT id, email, username, COALESCE(NULLIF(name,''), full_name) AS name, gigbit_insurance, vehicle_rented, daily_fuel, daily_rent, created_at FROM users WHERE id = $1", [userId]);
    if (!r.rowCount)
...
app.post("/user/expense-settings", async (req, res) => {
    ...
    const nextVehicleRented = vehicleRentedRaw == null ? currentVehicleRented : Boolean(vehicleRentedRaw);
    ...
    await pgPool.query("UPDATE users SET vehicle_rented = $2, daily_fuel = $3, daily_rent = $4 WHERE id = $1", [userId, nextVehicleRented, dailyFuel, nextVehicleRented ? dailyRent : null]);
    res.json({
        message: "Updated",
        dailyFuel,
...
app.post("/account/delete-request", async (req, res) => {
    ...
    const r = await pgPool.query("INSERT INTO account_deletion_requests (user_id, user_email, reason_code, reason_text, status) VALUES ($1,$2,$3,$4,'pending') RETURNING id, status, created_at", [userId, email, reasonCode, reasonText || null]);
...
app.post("/auth/register", async (req, res) => {
    ...
    const created = await pgPool.query("INSERT INTO users (email, password_hash, full_name, name) VALUES ($1,$2,$3,$3) RETURNING id,email,name,full_name", [email, hash, fullName]);
...
app.post("/auth/register/request-otp", async (req, res) => {
    const email = String(req.body?.email ?? "").trim().toLowerCase();
    if (!email)
        return res.status(400).json({ message: "Email is required", detail: "Email is required" });
    const existing = await pgPool.query("SELECT id FROM users WHERE email = $1", [email]);
    if (existing.rowCount) {
        return res.status(409).json({ message: "Email already registered", detail: "Email already registered" });
    }
...
app.post("/auth/register/complete", async (req, res) => {
    ...
    const existing = await pgPool.query("SELECT id FROM users WHERE lower(email) = lower($1) OR lower(username) = lower($2)", [email, username]);
    if (existing.rowCount) {
        return res.status(409).json({ message: "Email or username already registered", detail: "Email or username already registered" });
    }
    const hash = await bcrypt.hash(password, 10);
    const created = await pgPool.query("INSERT INTO users (email, username, password_hash, full_name, name, vehicle_rented, gigbit_insurance, daily_fuel, daily_rent)" +
        " VALUES ($1,$2,$3,$4,$4,$5,$6,$7,$8) RETURNING id,email,username,name,full_name", [email, username, hash, fullName, vehicleRented, gigbitInsurance, dailyFuel, dailyRent]);
    const user = created.rows[0];
...
app.post("/auth/login", async (req, res) => {
    const identifier = String(req.body?.identifier ?? req.body?.email ?? req.body?.username ?? "")
        ...
        .toLowerCase();
    const password = String(req.body?.password ?? "");
    const row = await pgPool.query("SELECT id,email,username,COALESCE(NULLIF(name,''), full_name) AS name,password_hash FROM users WHERE lower(email) = lower($1) OR lower(username) = lower($1) LIMIT 1", [identifier]);
    if (!row.rowCount)
        return res.status(401).json({ message: "Invalid credentials", detail: "Invalid credentials" });
...
app.post("/auth/password-reset/request", async (req, res) => {
    ...
    if (!row.rowCount)
        return res.status(404).json({ message: "Email not found", detail: "Email not found" });
    const otp = genOtp();
    await setOtp(email, otp);
    let mailMeta = null;
    try {
...
app.post("/auth/verify-otp", async (req, res) => {
    ...
    if (existing.rowCount)
        return res.status(409).json({ message: "Email already registered", detail: "Email already registered" });
    const hash = await bcrypt.hash(password, 10);
    const created = await pgPool.query("INSERT INTO users (email,password_hash,full_name,name,username) VALUES ($1,$2,$3,$3,$4) RETURNING id,email,name,full_name,username", [email, hash, name, null]);
    const user = created.rows[0];
    await clearOtp(email);
...
app.get("/user/platforms/:userId", async (req, res) => {
    ...
    if (!userId)
        return res.status(400).json({ message: "User required", detail: "User required" });
    await refreshSubscriptionRuntimeState(userId);
    res.json({ platforms: await getPlatforms(userId) });
});
app.get("/platforms/catalog", async (_req, res) => {
    const r = await pgPool.query("SELECT id, slug, name, logo_url, logo_bg_color, enabled, sort_order FROM integration_platform_catalog WHERE enabled = TRUE ORDER BY sort_order ASC, created_at ASC");
...
app.get("/user/approval-updates/stream", async (req, res) => {
    ...
    req.on("close", () => {
        ...
        if (set) {
            set.delete(res);
            if (!set.size)
                userApprovalStreamClients.delete(userId);
        }
        res.end();
    });
});
app.post("/user/push-token", async (req, res) => {
    ...
    await pgPool.query(`INSERT INTO user_push_tokens (user_id, token, platform, is_active, created_at, updated_at)
...
app.delete("/user/push-token", async (req, res) => {
    ...
    await pgPool.query("UPDATE user_push_tokens SET is_active = FALSE, updated_at = NOW() WHERE user_id = $1 AND token = $2", [userId, token]);
...
app.post("/platforms/connect", async (req, res) => {
    ...
    if (!userId || !platform)
        return res.status(400).json({ message: "Invalid input", detail: "Invalid input" });
    const platformCatalog = await pgPool.query("SELECT enabled FROM integration_platform_catalog WHERE lower(slug) = $1 LIMIT 1", [platform]);
    ...
    const existingConn = await pgPool.query("SELECT 1 FROM platform_connections WHERE user_id = $1 AND lower(platform) = $2", [userId, platform]);
    if (existingConn.rowCount) {
        return res.json({ message: "Platform connected" });
    }
    const existingBinding = await pgPool.query(`SELECT b.id
       FROM platform_subscription_bindings b
       JOIN subscription_purchases sp ON sp.id = b.purchase_id
      WHERE b.user_id = $1
    ...
    if (!existingBinding.rowCount) {
        const usageR = await pgPool.query(`SELECT b.purchase_id, COUNT(*)::int AS c
       FROM platform_subscription_bindings b
       JOIN subscription_purchases sp ON sp.id = b.purchase_id
       WHERE b.user_id = $1
         AND b.unbound_at IS NULL
       ...
       GROUP BY b.purchase_id`, [userId]);
        ...
        await pgPool.query(`INSERT INTO platform_subscription_bindings
        (user_id, platform, purchase_id, plan, bound_at, expires_at)
       VALUES ($1,$2,$3,$4,NOW(),$5)
       ON CONFLICT (user_id, platform) WHERE unbound_at IS NULL DO NOTHING`, [userId, platform, slot.id, slot.plan, slot.expiresAt]);
        await pgPool.query("INSERT INTO platform_connection_history (user_id,platform) VALUES ($1,$2) ON CONFLICT (user_id,platform) DO NOTHING", [userId, platform]);
    }
    await pgPool.query("INSERT INTO platform_connections (user_id,platform) VALUES ($1,$2) ON CONFLICT (user_id,platform) DO NOTHING", [userId, platform]);
...
app.post("/platforms/disconnect", async (req, res) => {
    const userId = req.auth?.userId ?? String(req.body?.user_id ?? "").trim();
    const platform = String(req.body?.platform ?? "").trim().toLowerCase();
    if (!userId || !platform)
        return res.status(400).json({ message: "Invalid input", detail: "Invalid input" });
...
app.post("/platforms/sync-earning", async (req, res) => {
    ...
    const todayR = await pgPool.query("SELECT COALESCE(SUM(amount),0) AS total FROM platform_earnings WHERE user_id = $1 AND platform = $2 AND ((created_at AT TIME ZONE 'Asia/Kolkata')::date = (NOW() AT TIME ZONE 'Asia/Kolkata')::date)", [userId, platform]);
...
app.get("/transactions", async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    const r = await pgPool.query(`SELECT t.id,t.platform,t.amount,t.note,t.created_at
     FROM transactions t
...
app.get("/insurance/claims", async (req, res) => {
    ...
    const r = await pgPool.query(`SELECT id, claim_type, incident_date, claim_amount, proof_name, proof_url, status, created_at
...
app.post("/expenses/upsert-daily", async (req, res) => {
    ...
    const category = categoryRaw.includes("rent")
        ...
        : (categoryRaw.includes("fuel") || categoryRaw.includes("petrol") || categoryRaw.includes("diesel"))
            ? "fuel"
            : categoryRaw;
    const updated = await pgPool.query(`UPDATE expenses
       SET amount = $3, note = $4
     WHERE id = (
       SELECT id
       FROM expenses
     ...
     RETURNING id, category, amount, note, created_at`, [userId, category, amount, note]);
    if (updated.rowCount) {
        return res.json({ message: "Expense updated", expense: updated.rows[0] });
    }
    const inserted = await pgPool.query("INSERT INTO expenses (user_id, category, amount, note) VALUES ($1,$2,$3,$4) RETURNING id, category, amount, note, created_at", [userId, category, amount, note]);
...
app.get("/expenses", async (req, res) => {
    ...
    const r = await pgPool.query("SELECT id, category, amount, note, created_at FROM expenses WHERE user_id = $1 ORDER BY created_at DESC LIMIT 500", [userId]);
...
app.get("/loan/requests", async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    const r = await pgPool.query(`SELECT id, amount, status, created_at
...
app.post("/loan/apply", async (req, res) => {
    ...
    const r = await pgPool.query(`INSERT INTO loans
...
app.post("/support/tickets", async (req, res) => {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const r = await pgPool.query("INSERT INTO support_tickets (ticket_number, user_id, subject, complaint, status, updated_at) VALUES ($1,$2,$3,$4,'open',NOW()) ON CONFLICT (ticket_number) DO NOTHING RETURNING id, ticket_number, subject, complaint, status, created_at, updated_at", [ticketNumber, userId, subject || "Support Ticket", complaint]);
        if (r.rowCount) {
            created = r.rows[0];
            break;
        }
...
app.get("/tax/assistant/history", async (req, res) => {
    const userId = req.auth?.userId ?? String(req.query?.user_id ?? "").trim();
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    ...
    const r = chatId === "__legacy__"
        ? await pgPool.query(`SELECT id, question, answer, created_at, session_id
...
app.get("/tax/assistant/chats", async (req, res) => {
    ...
    const r = await pgPool.query(`SELECT s.id,
     ...
     LIMIT $2`, [userId, limit]);
    const chats = [...r.rows];
    const legacy = await pgPool.query("SELECT COUNT(*)::int AS c, MAX(created_at) AS last_message_at FROM tax_chat_messages WHERE user_id = $1 AND session_id IS NULL", [userId]);
...
app.post("/tax/assistant/chats", async (req, res) => {
    ...
    const r = await pgPool.query(`INSERT INTO tax_chat_sessions (user_id, title)
     VALUES ($1, $2)
     RETURNING id, title, created_at, updated_at`, [userId, title]);
    res.status(201).json({ chat: r.rows[0] });
});
const renameTaxChatHandler = async (req, res) => {
    const userId = req.auth?.userId ?? String(req.body?.user_id ?? "").trim();
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    ...
    const r = await pgPool.query(`UPDATE tax_chat_sessions
     SET title = $3, updated_at = NOW()
     WHERE id = $1 AND user_id = $2
     RETURNING id, title, created_at, updated_at`, [chatId, userId, title]);
    if (!r.rowCount) {
...
const deleteTaxChatHandler = async (req, res) => {
    const userId = req.auth?.userId ?? String(req.body?.user_id ?? "").trim();
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    const chatId = String(req.params.id ?? "").trim();
    ...
    if (chatId === "__legacy__") {
        const r = await pgPool.query("DELETE FROM tax_chat_messages WHERE user_id = $1 AND session_id IS NULL", [userId]);
        return res.json({ message: "Chat deleted", deletedMessages: r.rowCount ?? 0 });
    }
    const deletedMessages = await pgPool.query("DELETE FROM tax_chat_messages WHERE user_id = $1 AND session_id = $2", [userId, chatId]);
    const deletedChat = await pgPool.query("DELETE FROM tax_chat_sessions WHERE id = $1 AND user_id = $2 RETURNING id", [chatId, userId]);
...
app.post("/tax/assistant", async (req, res) => {
    ...
    if (!chatId) {
        ...
        const created = await pgPool.query(`INSERT INTO tax_chat_sessions (user_id, title)
       VALUES ($1, $2)
       RETURNING id, title`, [userId, firstTitle || "New Chat"]);
        chatId = String(created.rows[0]?.id ?? "").trim();
    }
    if (chatId && chatId !== "__legacy__") {
        const owns = await pgPool.query("SELECT id, title FROM tax_chat_sessions WHERE id = $1 AND user_id = $2 LIMIT 1", [chatId, userId]);
        if (!owns.rowCount) {
            return res.status(404).json({ message: "Chat not found", detail: "Chat not found" });
        }
        ...
        if (!currentTitle || currentTitle.toLowerCase() === "new chat") {
            ...
            await pgPool.query("UPDATE tax_chat_sessions SET title = $3, updated_at = NOW() WHERE id = $1 AND user_id = $2", [chatId, userId, nextTitle]);
        }
        else {
            await pgPool.query("UPDATE tax_chat_sessions SET updated_at = NOW() WHERE id = $1 AND user_id = $2", [chatId, userId]);
        }
    }
    ...
    let answer = "";
    if (!inScope) {
        answer = taxT(lang, "I can only help with Tax, ITR filing, and GigBit platform queries. Please ask about deductions, TDS, ITR form selection, filing steps, or GigBit earnings/expense tax summary.", "मैं केवल Tax, ITR filing और GigBit platform से जुड़े सवालों में मदद कर सकता हूँ। कृपया deductions, TDS, ITR form, filing steps या GigBit earnings/expense tax summary के बारे में पूछें।", "मी फक्त Tax, ITR filing आणि GigBit platform संबंधित प्रश्नांमध्ये मदत करू शकतो. कृपया deductions, TDS, ITR form, filing steps किंवा GigBit earnings/expense tax summary बद्दल विचारा.");
        await pgPool.query("INSERT INTO tax_chat_messages (user_id, session_id, question, answer) VALUES ($1,$2,$3,$4)", [userId, chatId && chatId !== "__legacy__" ? chatId : null, question, answer]);
    ...
    const answerLines = [
        taxT(lang, "Tax Assistant Scope: Tax, ITR, and GigBit platform finances only.", "टैक्स असिस्टेंट का दायरा: केवल Tax, ITR और GigBit platform finance.", "टॅक्स असिस्टंटचा विषय: फक्त Tax, ITR आणि GigBit platform finance."),
        "",
        taxT(lang, `Quick Answer: Estimated taxable business income is about Rs ${Math.round(taxable)}.`, `त्वरित उत्तर: अनुमानित करयोग्य व्यवसाय आय लगभग Rs ${Math.round(taxable)} है।`, `झटपट उत्तर: अंदाजे करपात्र व्यवसाय उत्पन्न Rs ${Math.round(taxable)} आहे.`),
        taxT(lang, "Keep expense proofs and verify final liability with a CA before filing.", "खर्च के प्रमाण रखें और फाइलिंग से पहले अंतिम टैक्स देनदारी CA से सत्यापित करें।", "खर्चाचे पुरावे जतन करा आणि फायलिंगपूर्वी अंतिम करदेयता CA कडून तपासा."),
        "",
        taxT(lang, "ITR Filing Summary (Use This On Official ITR Portal)", "ITR फाइलिंग सारांश (इसे आधिकारिक ITR पोर्टल पर उपयोग करें)", "ITR फायलिंग सारांश (हे अधिकृत ITR पोर्टलवर वापरा)"),
        taxT(lang, `Financial Year (FY): ${fyStartYear}-${String(fyEndYear).slice(-2)}`, `वित्तीय वर्ष (FY): ${fyStartYear}-${String(fyEndYear).slice(-2)}`, `आर्थिक वर्ष (FY): ${fyStartYear}-${String(fyEndYear).slice(-2)}`),
        taxT(lang, `Assessment Year (AY): ${ayStartYear}-${String(ayEndYear).slice(-2)}`, `आकलन वर्ष (AY): ${ayStartYear}-${String(ayEndYear).slice(-2)}`, `आकलन वर्ष (AY): ${ayStartYear}-${String(ayEndYear).slice(-2)}`),
        taxT(lang, "Taxpayer Type: Individual", "करदाता प्रकार: व्यक्तिगत", "करदाता प्रकार: वैयक्तिक"),
        taxT(lang, "Income Nature: Gig/Platform Income (Business or Profession)", "आय का प्रकार: Gig/Platform आय (व्यवसाय या पेशा)", "उत्पन्न प्रकार: Gig/Platform उत्पन्न (व्यवसाय किंवा व्यावसायिक)"),
        taxT(lang, `Gross Receipts (From GigBit): Rs ${Math.round(summary.totalEarnings)}`, `कुल प्राप्तियां (GigBit से): Rs ${Math.round(summary.totalEarnings)}`, `एकूण प्राप्ती (GigBit मधून): Rs ${Math.round(summary.totalEarnings)}`),
        taxT(lang, `Allowable Expenses (From GigBit): Rs ${Math.round(summary.expenses)}`, `स्वीकार्य खर्च (GigBit से): Rs ${Math.round(summary.expenses)}`, `मान्य खर्च (GigBit मधून): Rs ${Math.round(summary.expenses)}`),
        taxT(lang, `Net Taxable Income (Estimate): Rs ${Math.round(taxable)}`, `शुद्ध करयोग्य आय (अनुमान): Rs ${Math.round(taxable)}`, `निव्वळ करपात्र उत्पन्न (अंदाज): Rs ${Math.round(taxable)}`),
        taxT(lang, "TDS Credit: Fill from Form 26AS / AIS", "TDS क्रेडिट: Form 26AS / AIS से भरें", "TDS क्रेडिट: Form 26AS / AIS मधून भरा"),
        taxT(lang, "Advance/Self-Assessment Tax: Fill actual challan values", "एडवांस/सेल्फ-असेसमेंट टैक्स: वास्तविक चालान मान भरें", "Advance/Self-Assessment Tax: प्रत्यक्ष चलन मूल्य भरा"),
        taxT(lang, "Suggested Form: ITR-3 or ITR-4 (if eligible under presumptive scheme)", "सुझाया गया फॉर्म: ITR-3 या ITR-4 (यदि presumptive scheme के तहत पात्र हों)", "सुचवलेला फॉर्म: ITR-3 किंवा ITR-4 (presumptive scheme अंतर्गत पात्र असल्यास)"),
        "",
        taxT(lang, `Original Question: ${question}`, `मूल प्रश्न: ${question}`, `मूळ प्रश्न: ${question}`),
    ];
...
app.get("/subscription/purchases", async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    const r = await pgPool.query("SELECT id, plan, amount, created_at, (created_at + INTERVAL '30 days') AS expires_at FROM subscription_purchases WHERE user_id = $1 ORDER BY created_at DESC LIMIT 120", [userId]);
...
app.get("/subscription/:userId", async (req, res) => {
    ...
    const pendingPlan = r.rowCount ? r.rows[0].pending_plan : null;
    const pendingActivationAt = r.rowCount ? r.rows[0].updated_at : null;
    res.json({
...
app.post("/subscription/select", async (req, res) => {
    ...
    if (!userId || !inc || !amount)
        return res.status(400).json({ message: "Invalid plan", detail: "Invalid plan" });
    const maxPlatforms = await getCatalogPlatformCapacity();
    ...
    const monthStartR = await pgPool.query("SELECT (date_trunc('month', timezone('Asia/Kolkata', NOW())) AT TIME ZONE 'Asia/Kolkata') AS m");
    const monthStart = monthStartR.rows[0].m;
    const countR = await pgPool.query("SELECT COUNT(*)::int AS c FROM subscription_purchases WHERE user_id = $1 AND plan = $2 AND created_at >= $3 AND created_at < ($3 + INTERVAL '1 month')", [userId, plan, monthStart]);
    ...
    await pgPool.query("INSERT INTO subscriptions (user_id,pending_plan,status) VALUES ($1,$2,'pending_payment') ON CONFLICT (user_id) DO UPDATE SET pending_plan = EXCLUDED.pending_plan, status = 'pending_payment', updated_at = NOW()", [userId, plan]);
...
app.post("/subscription/confirm-mock-payment", async (req, res) => {
    ...
    if (!r.rowCount || !r.rows[0].pending_plan)
        return res.status(400).json({ message: "No pending plan", detail: "No pending plan" });
    ...
    await pgPool.query("INSERT INTO subscription_purchases (user_id, plan, amount) VALUES ($1,$2,$3)", [userId, plan, planAmount(plan)]);
    await pgPool.query("UPDATE subscriptions SET pending_plan = NULL, updated_at = NOW() WHERE user_id = $1", [userId]);
    const nextState = await refreshSubscriptionRuntimeState(userId);
    res.json({
...
app.post("/users/:userId/profile", async (req, res) => {
    ...
    if (!name)
        return res.status(400).json({ message: "Name required", detail: "Name required" });
...
app.post("/user/profile/password/verify-old", async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    if (!oldPassword)
        return res.status(400).json({ message: "Old password required", detail: "Old password required" });
    ...
    await pgPool.query("INSERT INTO profile_password_verifications (token, user_id, expires_at) VALUES ($1,$2,NOW()+INTERVAL '10 minutes')", [token, userId]);
...
app.post("/user/profile/password/update", async (req, res) => {
    ...
    if (newPassword.length < 8)
        return res.status(400).json({ message: "New password too short", detail: "New password too short" });
    if (!verifyToken)
        return res.status(400).json({ message: "Verify token required", detail: "Verify token required" });
    const v = await pgPool.query("SELECT token FROM profile_password_verifications WHERE user_id = $1 AND token = $2 AND expires_at >= NOW() LIMIT 1", [userId, verifyToken]);
    if (!v.rowCount)
        return res.status(401).json({ message: "Old password verification required", detail: "Old password verification required" });
    const hash = await bcrypt.hash(newPassword, 10);
...
app.post("/user/profile/email/change/request-old-otp", async (req, res) => {
    ...
    const oldOtp = genOtp();
    await pgPool.query("INSERT INTO user_email_change_flows (id, user_id, old_email, old_otp, old_otp_expires_at, status) VALUES ($1,$2,$3,$4,NOW()+INTERVAL '10 minutes','pending_old')", [flowId, userId, oldEmail, oldOtp]);
    try {
...
app.post("/user/profile/email/change/verify-old", async (req, res) => {
    ...
    if (!userId)
        return res.status(401).json({ message: "Unauthorized", detail: "Unauthorized" });
    if (!flowId || !otp || !newEmail)
        return res.status(400).json({ message: "Invalid input", detail: "Invalid input" });
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newEmail)) {
        return res.status(400).json({ message: "Invalid email", detail: "Invalid email" });
    }
    const f = await pgPool.query("SELECT id, old_email, old_otp, old_otp_expires_at, status FROM user_email_change_flows WHERE id = $1 AND user_id = $2 LIMIT 1", [flowId, userId]);
    if (!f.rowCount)
        return res.status(404).json({ message: "Flow not found", detail: "Flow not found" });
    const flow = f.rows[0];
    ...
    if (exists.rowCount)
        return res.status(409).json({ message: "Email already registered", detail: "Email already registered" });
    const newOtp = genOtp();
    await pgPool.query("UPDATE user_email_change_flows SET old_verified = TRUE, new_email = $3, new_otp = $4, new_otp_expires_at = NOW()+INTERVAL '10 minutes', status = 'pending_new', updated_at = NOW() WHERE id = $1 AND user_id = $2", [flowId, userId, newEmail, newOtp]);
    try {
...
app.post("/user/profile/email/change/verify-new", async (req, res) => {
    ...
    if (!flowId || !otp)
        return res.status(400).json({ message: "Invalid input", detail: "Invalid input" });
    const f = await pgPool.query("SELECT id, new_email, new_otp, new_otp_expires_at, status FROM user_email_change_flows WHERE id = $1 AND user_id = $2 LIMIT 1", [flowId, userId]);
    if (!f.rowCount)
        return res.status(404).json({ message: "Flow not found", detail: "Flow not found" });
    const flow = f.rows[0];
    ...
    if (!newEmail)
        return res.status(400).json({ message: "Invalid flow", detail: "Invalid flow" });
    ...
    await pgPool.query("UPDATE user_email_change_flows SET new_verified = TRUE, status = 'completed', updated_at = NOW() WHERE id = $1 AND user_id = $2", [flowId, userId]);
...
async function processWithdraw(req, res) {
    ...
    const r = await pgPool.query("INSERT INTO withdrawals (user_id,amount,insurance_contribution,service_fee,total_fee,user_receives) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *", [userId, amount, insurance, fee, totalFee, amount]);
...
async function submitClaim(req, res) {
    try {
        ...
        const capR = await pgPool.query(`SELECT COUNT(*)::int AS c
       FROM insurance_claims
       WHERE user_id = $1
         AND claim_type = $2
         AND EXTRACT(YEAR FROM incident_date) = EXTRACT(YEAR FROM $3::date)`, [userId, claimType, dateOnly]);
        ...
        const r = await pgPool.query(`INSERT INTO insurance_claims
        (user_id,claim_type,description,proof_url,proof_name,incident_date,claim_amount,status)
       VALUES ($1,$2,$3,$4,$5,$6,$7,'submitted')
       RETURNING id,user_id,claim_type,description,proof_url,proof_name,incident_date,claim_amount,status,created_at`, [userId, claimType, finalDescription, proofUrl, proofName || null, dateOnly, rules.amount]);
        res.status(201).json(r.rows[0]);
    }
    catch (error) {
...
async function getPlatforms(userId) {
    const r = await pgPool.query(`SELECT pc.platform
     FROM platform_connections pc
     JOIN integration_platform_catalog c ON lower(c.slug) = lower(pc.platform) AND c.enabled = TRUE
     WHERE pc.user_id = $1
     ORDER BY pc.connected_at DESC`, [userId]);
    return r.rows.map((x) => String(x.platform));
}
async function getLedger(userId) {
    const r = await pgPool.query(`SELECT id, entry_type, platform, amount, description, created_at FROM (
     ...
     ) x ORDER BY created_at DESC LIMIT 200`, [userId]);
    return r.rows;
}
async function getWithdrawalLimitState(userId) {
    const accruedR = await pgPool.query(`SELECT
      COALESCE(SUM(
        2 * GREATEST(
          0,
          LEAST(
    ...
    const usedR = await pgPool.query("SELECT COUNT(*)::int AS c FROM withdrawals WHERE user_id = $1 AND COALESCE(user_receives,0) > 0", [userId]);
...
async function ensureMonthlyInsuranceAutoDebit(userId) {
    ...
    if (!u.rowCount || !Boolean(u.rows[0].gigbit_insurance))
        return;
    const monthR = await pgPool.query("SELECT (date_trunc('month', timezone('Asia/Kolkata', NOW())) AT TIME ZONE 'Asia/Kolkata') AS m");
...
async function getLoan(userId) {
    const nowR = await pgPool.query("SELECT (NOW() AT TIME ZONE 'Asia/Kolkata')::date AS today_ist_date");
    const userR = await pgPool.query("SELECT (created_at AT TIME ZONE 'Asia/Kolkata')::date AS user_start_ist_date FROM users WHERE id = $1 LIMIT 1", [userId]);
    ...
    const userStart = userR.rowCount
        ...
        : today;
    const tenureDays = Math.max(1, Math.floor((today.getTime() - userStart.getTime()) / (24 * 60 * 60 * 1000)) + 1);
    ...
    const consideredDays = Math.max(1, Math.floor((today.getTime() - effectiveStart.getTime()) / (24 * 60 * 60 * 1000)) + 1);
    const dailyR = await pgPool.query(`SELECT (created_at AT TIME ZONE 'Asia/Kolkata')::date AS d, COALESCE(SUM(amount),0)::numeric AS total
     FROM transactions
     WHERE user_id = $1
       AND (created_at AT TIME ZONE 'Asia/Kolkata')::date BETWEEN $2::date AND $3::date
     GROUP BY (created_at AT TIME ZONE 'Asia/Kolkata')::date`, [userId, effectiveStart.toISOString().slice(0, 10), today.toISOString().slice(0, 10)]);
    let workedDays = 0;
...
function registerVerifiedKey(email) {
    return "register:verified:" + email;
}
async function markRegisterVerified(email) {
...
async function isRegisterVerified(email) {
...
async function clearRegisterVerified(email) {
    registerVerifiedMemory.delete(email);
    if (!redis.isOpen)
        return;
    try {
...
async function setOtp(email, otp) {
    await pgPool.query("INSERT INTO password_reset_otps (email,otp,expires_at) VALUES ($1,$2,NOW()+INTERVAL '10 minutes') ON CONFLICT (email) DO UPDATE SET otp = EXCLUDED.otp, expires_at = EXCLUDED.expires_at, created_at = NOW()", [email, otp]);
}
...
app.get("/admin/account-deletions", requireAdminKey, async (req, res) => {
    ...
    const r = await pgPool.query("SELECT id, user_id, user_email, reason_code, reason_text, status, created_at, reviewed_at FROM account_deletion_requests WHERE status = $1 ORDER BY created_at DESC LIMIT 200", [st]);
    res.json({ items: r.rows });
});
app.post("/admin/account-deletions/:id/approve", requireAdminKey, async (req, res) => {
    ...
    try {
        await client.query("BEGIN");
        const row = await client.query("SELECT id, user_id, status FROM account_deletion_requests WHERE id = $1 FOR UPDATE", [id]);
        if (!row.rowCount) {
            await client.query("ROLLBACK");
            return res.status(404).json({ message: "Not found", detail: "Not found" });
        }
        const reqRow = row.rows[0];
        await client.query("UPDATE account_deletion_requests SET status = 'approved', reviewed_at = NOW(), reviewed_by = $2, admin_note = $3 WHERE id = $1", [id, reviewedBy, note || null]);
        await client.query("COMMIT");
...
app.post("/admin/account-deletions/:id/reject", requireAdminKey, async (req, res) => {
    ...
    const note = String(req.body?.adminNote ?? req.body?.admin_note ?? "").trim();
    const reviewedBy = String(req.body?.reviewedBy ?? req.body?.reviewed_by ?? "admin").trim();
    await pgPool.query("UPDATE account_deletion_requests SET status = 'rejected', reviewed_at = NOW(), reviewed_by = $2, admin_note = $3 WHERE id = $1", [id, reviewedBy, note || null]);
    const row = await pgPool.query("SELECT user_id FROM account_deletion_requests WHERE id = $1", [id]);
...
app.get("/admin/loans/:id/repayments", requireAdminKey, async (req, res) => {
    ...
    const loanR = await pgPool.query(`SELECT id, user_id, tenure_months, monthly_installment, created_at
     FROM loans
     WHERE id = $1
     LIMIT 1`, [id]);
    if (!loanR.rowCount) {
    ...
    const payR = await pgPool.query(`SELECT installment_no, paid_at
     FROM loan_repayments
     WHERE loan_id = $1
     ORDER BY installment_no ASC`, [id]);
...
app.post("/admin/loans/:id/status", requireAdminKey, async (req, res) => {
    ...
    const r = await pgPool.query("UPDATE loans SET status = $2 WHERE id = $1 RETURNING id, user_id, amount, status, created_at", [id, status]);
    if (!r.rowCount)
        return res.status(404).json({ message: "Not found", detail: "Not found" });
    const item = r.rows[0];
...
app.post("/admin/insurance-claims/:id/status", requireAdminKey, async (req, res) => {
    ...
    const r = await pgPool.query("UPDATE insurance_claims SET status = $2 WHERE id = $1 RETURNING id, user_id, claim_type, status, created_at", [id, status]);
    if (!r.rowCount)
        return res.status(404).json({ message: "Not found", detail: "Not found" });
    const item = r.rows[0];
...
app.get("/admin/activity-logs", requireAdminKey, async (req, res) => {
    ...
    const r = await pgPool.query(`SELECT id, actor_username, action, details, created_at
...
app.get("/admin/commission-share", requireAdminKey, async (req, res) => {
    ...
    const useRequestedMonth = monthYear != null &&
    ...
    const byPlatform = await pgPool.query(`WITH month_bounds AS (SELECT ${monthBoundsSql}),
     ...
     ORDER BY lower(c.slug) ASC`, monthParams);
    ...
    const totals = await pgPool.query(`WITH month_bounds AS (SELECT ${monthBoundsSql})
     SELECT
       COALESCE((
    ...
    const monthsR = await pgPool.query(`SELECT to_char(month_key, 'YYYY-MM') AS month
     ...
     LIMIT 36`);
    const availableMonths = monthsR.rows.map((x) => String(x.month ?? "")).filter(Boolean);
    const selectedMonth = useRequestedMonth
        ? `${monthYear}-${String(monthNum).padStart(2, "0")}`
        : (availableMonths[0] ?? "");
    const items = byPlatform.rows.map((r) => ({
...
app.get("/admin/platforms", requireAdminKey, async (_req, res) => {
    const r = await pgPool.query("SELECT id, slug, name, logo_url, logo_bg_color, enabled, sort_order, created_at, updated_at FROM integration_platform_catalog ORDER BY sort_order ASC, created_at ASC");
    res.json({ items: r.rows });
});
app.post("/admin/platforms", requireAdminKey, async (req, res) => {
    ...
    if (!name)
        return res.status(400).json({ message: "Name is required", detail: "Name is required" });
    const slug = slugifyPlatformName(name);
    if (!slug)
        return res.status(400).json({ message: "Invalid platform name", detail: "Invalid platform name" });
    ...
    const r = await pgPool.query("INSERT INTO integration_platform_catalog (slug, name, logo_url, logo_bg_color, enabled, sort_order, updated_at) VALUES ($1,$2,$3,$4,$5,$6,NOW()) RETURNING id, slug, name, logo_url, logo_bg_color, enabled, sort_order, created_at, updated_at", [slug, name, logoUrl || null, logoBgColor || "#1E3A8A", enabled, nextSort]);
...
app.put("/admin/platforms/:id", requireAdminKey, async (req, res) => {
    ...
    const r = await pgPool.query("UPDATE integration_platform_catalog SET slug = $2, name = $3, logo_url = $4, logo_bg_color = $5, enabled = $6, updated_at = NOW() WHERE id = $1 RETURNING id, slug, name, logo_url, logo_bg_color, enabled, sort_order, created_at, updated_at", [id, nextSlug, nextName, nextLogo || null, nextBg, nextEnabled]);
...
app.post("/admin/platforms/:id/toggle", requireAdminKey, async (req, res) => {
    ...
    if (!id)
        return res.status(400).json({ message: "Invalid request", detail: "Invalid request" });
    const r = await pgPool.query("UPDATE integration_platform_catalog SET enabled = NOT enabled, updated_at = NOW() WHERE id = $1 RETURNING id, slug, name, logo_url, logo_bg_color, enabled, sort_order, created_at, updated_at", [id]);
    if (!r.rowCount)
        return res.status(404).json({ message: "Not found", detail: "Not found" });
...
async function ensureSchema() {
    const userIdTypeRow = await pgPool.query("SELECT data_type FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'id' LIMIT 1");
    const userIdType = userIdTypeRow.rowCount &&
    ...
    await pgPool.query("DELETE FROM platform_earnings WHERE lower(platform) IN ('swiggy','uber')");
    await pgPool.query("DELETE FROM transactions WHERE lower(platform) IN ('swiggy','uber')");
    await pgPool.query("DELETE FROM platform_subscription_bindings WHERE lower(platform) IN ('swiggy','uber')");
    await pgPool.query("DELETE FROM platform_connection_history WHERE lower(platform) IN ('swiggy','uber')");
    await pgPool.query("DELETE FROM platform_connections WHERE lower(platform) IN ('swiggy','uber')");
    await pgPool.query("DELETE FROM integration_platform_catalog WHERE lower(slug) IN ('swiggy','uber')");
    // Backfill platform slot history from current connections (safe no-op after first run).
    await pgPool.query("INSERT INTO platform_connection_history (user_id,platform,first_connected_at) SELECT user_id,platform,connected_at FROM platform_connections ON CONFLICT (user_id,platform) DO NOTHING");
    ...
    for (let i = 0; i < AVAILABLE_PLATFORMS.length; i++) {
        ...
        await pgPool.query("INSERT INTO integration_platform_catalog (slug, name, enabled, sort_order, updated_at) VALUES ($1,$2,TRUE,$3,NOW()) ON CONFLICT (slug) DO NOTHING", [slug, name, i + 1]);
    }
    // Backfill name from legacy full_name if needed.
    await pgPool.query("DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='full_name') THEN UPDATE users SET name = COALESCE(NULLIF(name, ''), full_name) WHERE (name IS NULL OR name = '') AND full_name IS NOT NULL; END IF; END $$;");
}
async function ensureAdminAccount() {
    async function ensureNamedAdmin(username, password) {
        const canonical = username.trim();
        const normalized = canonical.toLowerCase();
        if (!canonical)
            return;
        const hash = await bcrypt.hash(password, 10);
        const existing = await pgPool.query("SELECT id FROM admin_users WHERE lower(username) = lower($1) LIMIT 1", [normalized]);
        if (existing.rowCount) {
            await pgPool.query("UPDATE admin_users SET username = $2, email = $3, password_hash = $4, updated_at = NOW() WHERE id = $1", [existing.rows[0].id, canonical, ADMIN_LOGIN_EMAIL, hash]);
            return;
        }
        await pgPool.query("INSERT INTO admin_users (username, email, password_hash, updated_at) VALUES ($1,$2,$3,NOW())", [canonical, ADMIN_LOGIN_EMAIL, hash]);
    }
